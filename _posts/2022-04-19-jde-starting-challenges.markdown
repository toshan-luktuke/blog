...
My initial thought was to use some sort of reverse heuristic approach to get the longest path. However, I quickly abandonded that idea as we had the always find the longest path and heuristic apporaches are not guaranteed to find this path. 
I settled on a recursive backtracking approach and it worked like a charm.
...
I decided to take up the PID Racecar Exercise.  
...
In the current ROS2 install, the sourcing is done via the installed repo
```bash
